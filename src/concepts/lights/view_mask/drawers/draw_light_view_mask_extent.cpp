...
ViewMask& ViewMask::draw_light_view_mask (
  ...
  MaskObjectList filtered = filter_lines_point_view(darena, segments[0].second, segments[0].first, screen_dims);
  fill_view_with_shadows (buffer, img->w, img->h, darena, filtered, segments[0].first.position);
  darena.complete_free_mo(filtered.obj);

  fill_remain_with_lights (buffer, img->w, img->h, segments[0].first);
