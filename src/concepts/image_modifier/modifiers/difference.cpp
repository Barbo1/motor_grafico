...
ImageModifier& ImageModifier::operator- (const ImageModifier& img) {
  ...
  for (int i = 0; i < std::min(this->texture->h, img.texture->h); i++) {
    ...
    for (int j = 0; j < std::min(this->texture->w, img.texture->w); j++) {
      SDL_GetRGBA (arr_i[i * img.texture->w + j], this->texture->format, &color.r, &color.g, &color.b, &a2);
      SDL_GetRGBA (*res, this->texture->format, &color.r, &color.g, &color.b, &color.a);
      *(res++) = SDL_MapRGBA (this->texture->format, color.r, color.g, color.b, 255 - a2);
    }
  }
