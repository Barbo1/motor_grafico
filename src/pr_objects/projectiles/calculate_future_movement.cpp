...
void Projectile::calculate_future_movement(Global* glb, const AngDir2 & extrenal_forces) {
  if (this->config & PCO_MOVIBLE) {
    ...
    this->direction_to_new_position.store(new_vel * coef_mult);
    this->multiplicative_distance = 1.f;
  }
}
