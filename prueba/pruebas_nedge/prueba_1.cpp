...
bool proper_array_distance (int32_t i, int32_t j, int32_t M) {
  if (i == 0 && j == M) {
    return true;
  } else if (i == M && j == 0) {
    return true;
  } else {
    return std::abs (i - j) < 2;
  }
}
...
int main () {
  ...
  std::array<std::pair<uint32_t, uint32_t>, points.size()> parts_bounds;
  std::array<uint32_t, 3*points.size()> parts_indexes;
  std::array<std::array<Dir2, 3>, points.size()> parts;
  std::array<uint32_t, 3*points.size()> aux_arr;
  uint32_t parts_bounds_size = 1;
  uint32_t parts_indexes_last = points.size() - 1;
  uint32_t total_parts = 1;
  for (uint32_t i = 0; i < points.size(); i++)
    parts_indexes[i] = i;
  parts_bounds[0] = {0, points.size() - 1};
  ...
  while (true) {
    ...
    std::cout << "  -> total: " << total_parts << std::endl;
    std::cout << "  -> bounds " << std::endl;
    std::cout << "    -> ";
    for (uint32_t i = 0; i < parts_bounds_size; i++) {
      std::cout << "(" << parts_bounds[i].first << ", " << parts_bounds[i].second << "), ";
    }
    std::cout << "  -> indexes " << std::endl;
    std::cout << "    -> ";
    for (uint32_t i = 0; i <= parts_indexes_last; i++) {
      std::cout << parts_indexes[i] << ", ";
    }
    std::cout << std::endl;
    uint32_t ind = 0;
    uint32_t first, last;
    ...
    std::cout << "encontrado con angulo obtuso: " << index_f;
    std::cout << ", en: " << ind << std::endl;
    ...
    for (int32_t i = 0; i < parts_size; i++) {
      ...
      if (proper_array_distance (index_f, i, parts_size-1) || (c1 > 0.0001f && c2 > 0.0001f))
        continue;

      std::cout << "pasa 1" << std::endl;
      for (int32_t j = 0; j < parts_size; j++) {
        if (proper_array_distance (index_f, j, parts_size-1))
          continue;
      ...
      if (new_dist < distance) {
        std::cout << "encontro: "<< i << " con distancia " << new_dist << std::endl;
        distance = new_dist;
        index_l = i;
      }
    ...
    index_l+=first;
    index_f+=first;
    std::cout << index_f << ", "<< index_l << std::endl;
    std::pair<uint32_t, uint32_t> max_min = std::minmax(index_l, index_f);
    index_f = max_min.first;
    index_l = max_min.second;
    ...
    parts_bounds[parts_bounds_size].first = elem2;
    parts_bounds[parts_bounds_size++].second = parts_bounds[ind].second + 2;
  ...
  std::cout << "***********************************" << std::endl;
  std::cout << "*  esto termina la primera parte  *"<< std::endl;
  std::cout << "***********************************" << std::endl;
  ...
  while (true) {
    ...
    int32_t index_l = 0, index_f = 0;
    for (int32_t i = 0; i < parts_size; i++) {
      const Dir2 vp = parts[index_f][1];
      const Dir2 vc = parts[index_f][2];
    ...
    Dir2 vp = part[1];
    Dir2 vc = part[2];
    bool angle_cond = (std::atan2(vc.percan() * vp, vp * vc) > 0.f) ^ clockwise;
    distance = INFINITY;
    for (int32_t i = 0; i < parts_size; i++) {
      Dir2 vec_new_try = parts[i][0] - given_point;
      float c1 = vp.pLd(vec_new_try, vc);
      float c2 = vc.pLd(vec_new_try, vp);
      std::cout << "-------------------------------" << std::endl;
      std::cout << "i: " << i << std::endl;
      std::cout << "c1: " << c1 << std::endl;
      std::cout << "c2: " << c2 << std::endl;
      if (proper_array_distance (index_f, i, parts_size-1) || ((c1 > 0.0001f && c2 > 0.0001f) ^ angle_cond))
        continue;
      ...
      if (proper_array_distance (index_f, j, parts_size-1))
        ...
        float c2 = -vec_new_try.pLd(diff, vec_line);
        if ((0.0001f < c1 && c1 < 0.9998f && 0.0001f < c2 && c2 < 0.9998f)) {
          goto FIN_DIST_PLOY_2;
        }
      }
      std::cout << "pasa 2" << std::endl;

      new_dist = (vp * vc) / (vp.modulo() * vc.modulo());
      if (new_dist < distance) {
        std::cout << "encontro: "<< i << " con distancia " << new_dist << std::endl;
        distance = new_dist;
        index_l = i;
