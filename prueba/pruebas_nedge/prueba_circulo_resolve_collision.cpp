...
int main () {
  ...
  NEdgeComp<10> poly(points.data(), points.size(), Dir2 (100.f, 100.f), 2.f, 0.f, true, &error);
  if (error < 0) {
    std::cout << "problema al cargar poligono." << std::endl;
    std::exit(-1);
  }
  poly.physical.set_position(Dir2 (100.f, 100.f));
  ...
  CircleElement cir = CircleElement {
    ...
    .physical_body = Circle (Dir2(360.f, 600.f), 30.f, 2.f, 0.f, true)
  };
  ...
  while (cont) {
    ...
    glb->begin_render();
      ...
      if (test_collision(cir.physical_body, poly.physical)) {
        resolve_collision(cir.physical_body, poly.physical, true);
      }

      if (test_collision(cir1.physical_body, poly.physical)) {
        resolve_collision(cir1.physical_body, poly.physical, true);
      }
